...
export const verifyRegisterationOtp = async (req, res) => {
  try {
    ...
    // Create role-specific profile and link via userData
    if (createdUser.role === "Doctor") {
      const doctorProfile = await Doctor.create({
        ...
        contact: { phone: "", email: createdUser.email },
        isActive: true,
      });
      createdUser.userData = doctorProfile._id;
      await createdUser.save();
    } else if (createdUser.role === "Patient") {
      const patientProfile = await Patient.create({
        userId: createdUser._id,
      });
      createdUser.userData = patientProfile._id;
      await createdUser.save();
    }
